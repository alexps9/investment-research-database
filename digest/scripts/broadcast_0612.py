#!/usr/bin/env python3
"""2026-06-12 三路广播：form A 卡(企业群 webhook + 飞书订阅 1-1) + H Link(文字=一杯咖啡那句，与周末一致)。
默认 dry-run；--send 真发；--hlink-all 用 G0+G1 全部 H Link 收件人(否则默认 OPENIDS)。"""
import os, sys
os.environ.setdefault("LARK_CLI_NO_PROXY", "1")
_H = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, _H)
from send_0612_form_a import card_a, DOC  # noqa: E402  (also loads .env + src path)
...
TITLE = "HH Research Daily · 2026-06-12"
