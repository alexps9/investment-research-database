...
<p>{anchor:top_2}</p>
<h3>英伟达 Blackwell Ultra GB300 在首个 AI 智能体专项基准中夺冠——每兆瓦能跑的并发智能体数是上代 H200 的 20 倍</h3>
<ul>
  <li><b>信号源</b>：<b>NVIDIA</b> 官方博客（<a href="https://blogs.nvidia.com/blog/nvidia-blackwell-agentperf-artificial-analysis/">NVIDIA Blog</a>）· <b>Rohan Paul</b>（<a href="https://x.com/rohanpaul_ai/status/2065576558312710584">X · 技术解读</a>）</li>
</ul>
<callout emoji="💡" background-color="light-orange" border-color="orange">
  <p><b>Insights</b></p>
  <p text-indent="1">衡量 AI 芯片的标尺正在从"每秒能吐多少字"（tokens/s）转向"每度电能同时养活多少个智能体"，这背后是 AI 应用形态从单轮问答转向多智能体长链调用的根本变化。<b>AgentPerf 是业内首个专为智能体负载设计的基础设施基准，它考的不是峰值吞吐，而是在保证响应时效的前提下、每兆瓦功耗能支撑的并发智能体数量——这正是数据中心算这笔账时最关心的指标。</b>GB300 NVL72 以每兆瓦 61,400 个并发编码智能体夺冠，约为上代 H200（2,600）的 20 倍，提升主要来自 72 颗 GPU 的机架级 NVLink 互联、专家路由的分布式调度，以及通信与计算的重叠优化——也就是说，这一代的优势不在单卡峰值，而在"整机协同"。对投研的含义是双重的：其一，单位算力的智能体承载量大幅跃升，会直接拉低 agentic 应用的边际推理成本；其二，AgentPerf 这类新基准一旦被接受，将成为 AMD、谷歌 TPU 等平台在智能体时代被对标的统一标尺，竞争评价体系正在重写。</p>
</callout>
<p text-indent="1"><b>基准与成绩：</b>第三方评测机构 Artificial Analysis 发布业内首个智能体 AI 基础设施基准 <b>AgentPerf</b>，核心指标是在严格延迟约束下、每兆瓦可支撑的最大并发 AI 智能体数量，覆盖 5K 至 131K token 的超长上下文，综合考量内存、网络、专家路由（MoE 模型内部的请求分发）与延迟。<b>英伟达 Blackwell Ultra GB300 NVL72</b> 在首轮测试中以每兆瓦 <b>61,400</b> 个并发编码智能体登顶，上代 <b>H200</b> 为每兆瓦 <b>2,600</b> 个，差距约 <b>20 倍</b>。</p>
<p text-indent="1"><b>架构来源：</b>该平台采用 72 颗 GPU 的 NVLink 机架级互联，配合通信与计算重叠优化，在多智能体、长上下文的真实负载下显著提升整机能效——这与单纯堆砌单卡算力是两条不同的工程路线。</p>
