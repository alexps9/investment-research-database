...
<callout emoji="📖" background-color="light-blue" border-color="blue">
  <p><b>拓展阅读：什么是扩散语言模型？（写给非技术读者）</b></p>
  <p><b>它从哪来：</b>"扩散模型"正是 Midjourney、Stable Diffusion 等 AI 画图工具背后的技术。训练时先学会"把一张清晰图片一步步加噪点、直到变成雪花屏"，再学会逆过程；生成时从一片随机噪点出发，一步步"去噪显影"出完整图像。</p>
  <p><b>用到文字上怎么办：</b>文字不是像素，所以文本扩散改用"填空"方式——把整段文字全部蒙住，模型多轮迭代、同时填所有空、每一轮还能修改之前填错的地方，直到整段成文。对比主流的自回归模型（GPT、Claude、Gemini 都是）：逐字往外蹦、写出的字不能回改。<b>一个是"整页同时显影、可反复修订"，一个是"逐字书写、落笔无悔"——这就是两条路线的本质区别。</b></p>
  <p><b>为什么投研要关心：</b>逐字生成的速度瓶颈在显存带宽（GPU 大量时间在等数据搬运），整块生成的瓶颈在算力本身。若扩散路线跑通，推理芯片的设计取向、推理服务的定价结构、单位 token 的成本曲线都可能被改写——这是"成本结构级"的变量，不是单一产品的输赢。</p>
  <p><b>研究路线一览：</b></p>
  <p>• 2020–2022：扩散模型在图像生成上成熟（DDPM → Stable Diffusion）；文本方向仅小规模学术探索</p>
  <p>• 2025.02：<b>LLaDA</b>（中国人民大学高瓴人工智能学院等，8B 从零训练）证明纯扩散语言模型可比肩同规模 LLaMA3——<b>路线可行</b>（<a href="https://arxiv.org/abs/2502.09992">论文</a>）</p>
  <p>• 2025：<b>Mercury</b>（Inception Labs，扩散模型共同发明人、Stanford 教授 Stefano Ermon 创立）首个商用级扩散语言模型，以"快至 10 倍"为卖点——<b>商业可行</b></p>
  <p>• 2025 年中：<b>Gemini Diffusion</b>（Google I/O 实验版）——大厂内部试水</p>
  <p>• 2026.06：<b>DiffusionGemma</b> 26B 开源——大厂背书 + 开放权重 + 生态 Day-0 支持，<b>竞赛进入生态卡位阶段</b></p>
  <p><b>当前局限（保持清醒）：</b>质量上限尚未追平最强自回归模型（Google 自己定位"优先速度而非质量"），长文一致性与复杂工具调用仍在早期。<b>下一个观察点：是否出现质量与速度兼得的扩散旗舰，以及 vLLM 等推理框架对扩散模型的原生优化深度。</b></p>
</callout>
