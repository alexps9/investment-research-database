"""v8.0 Headline selector (Plan v3 §6.1).

Takes a list of already-classified signals and decides:
  - which go to auto_headline (constraint_pass = True)
  - which go to edge_case (constraint_pass = False but m1+m2+m3+m4+m5 ≥ threshold)
...
log = get_logger("headline_selector")
...
class SelectionResult:
    auto_headlines: list[Signal]  # constraint_pass=True, deduplicated, ranked
    edge_cases: list[Signal]      # candidates for G0 review, ranked
    ...
    def select(self, signals: list[Signal]) -> SelectionResult:
        ...
        for key, group in groups.items():
            if len(group) == 1:
                ...
                continue
            # Same canonical_event_key → keep strongest (constraint_pass first, then m-sum)
            group_sorted = sorted(
                group,
        ...
        # Sort winners: auto first (by m-sum desc), then edge candidates
        winners_sorted = sorted(winners, key=lambda s: (s.constraint_pass, self._m_sum(s)), reverse=True)
        ...
        for s in winners_sorted:
            if s.constraint_pass and len(auto) < self.max_auto_per_day:
                ...
                # Already hit cap; demote to edge case
