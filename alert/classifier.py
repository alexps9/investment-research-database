...
返回严格 JSON：
{"should_alert": true/false, "alert_level": "important|medium", "event_type": "模型/产品发布|技术研究突破|硬件/Infra突破|大佬评论/观点|评测/榜单|Demo/演示|公司间商业动作|顶级人员变动|无", "confidence": "confirmed|reported|rumored", "reason": "一句话判别理由"}

字段说明：
- confidence：信源可信度。confirmed=官方一手/多源印证/有可核实文件；reported=有具体主体+数字但非当事方说的；rumored=单源+据报道/据悉/leak。
- reason：一句话解释为什么这么判（如"官方一手发布+3源印证+金额巨大→important"或"单源传闻+无时间表→medium"）。

should_alert=false 时 alert_level、event_type、confidence、reason 可以省略。"""
...
{tweet['text']}"""
    ...
    if skip_judge:
        result = {"should_alert": True, "alert_level": "medium", "event_type": "",
                  "confidence": "confirmed", "reason": "P0+ official, prefilter pass"}
    else:
    ...
    if not verified and (tier == "aggregator" or is_rumor) and result.get("alert_level") == "important":
        ...
        if result.get("confidence") == "confirmed":
            result["confidence"] = "reported"

    # Ensure confidence/reason always present (fallback if LLM omitted)
    if "confidence" not in result:
        if verified:
            result["confidence"] = "confirmed"
        elif is_rumor or tier == "aggregator":
            result["confidence"] = "rumored"
        else:
            result["confidence"] = "reported"
